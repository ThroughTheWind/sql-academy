...
public static class LearningDbSeed
{
    ...
    {
        var hasCoreSeed = await dbContext.Users.AnyAsync(cancellationToken);
        var hasTenantSampleSeed = await dbContext.TenantOrders.AnyAsync(cancellationToken);

        if (hasCoreSeed && hasTenantSampleSeed)
        {
        ...
        if (!hasTenantSampleSeed)
        {
            var tenantOrders = new[]
            {
                new TenantOrder { TenantId = 101, OrderNumber = "TEN-101-0001", Description = "Northwind renewal order", TotalAmount = 1250.00m, CreatedUtc = baseTimestamp.AddDays(7).AddMinutes(5) },
                new TenantOrder { TenantId = 101, OrderNumber = "TEN-101-0002", Description = "Northwind seat expansion", TotalAmount = 480.50m, CreatedUtc = baseTimestamp.AddDays(8).AddMinutes(10) },
                new TenantOrder { TenantId = 202, OrderNumber = "TEN-202-0001", Description = "Tailspin onboarding order", TotalAmount = 2200.00m, CreatedUtc = baseTimestamp.AddDays(7).AddMinutes(20) },
                new TenantOrder { TenantId = 202, OrderNumber = "TEN-202-0002", Description = "Tailspin services renewal", TotalAmount = 980.75m, CreatedUtc = baseTimestamp.AddDays(9).AddMinutes(15) },
            };

            await dbContext.TenantOrders.AddRangeAsync(tenantOrders, cancellationToken);
        }

        await dbContext.SaveChangesAsync(cancellationToken);

        logger.LogInformation("Seeded LearningDb with baseline academy data plus the tenant-isolation sample orders used by the RLS lab.");
    }
}