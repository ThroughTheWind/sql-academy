...
- the answer includes one concrete proof that tenant `101` and tenant `202` are isolated, one case where missing tenant context returns no rows, and one case where a cross-tenant write is blocked
